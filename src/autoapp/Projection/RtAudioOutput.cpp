...
bool RtAudioOutput::open()
{
    ...
    if(dac_->getDeviceCount() > 0)
    {
        ...
        if(dac_->openStream(&parameters, nullptr, RTAUDIO_SINT16, sampleRate_, &bufferFrames, &RtAudioOutput::audioBufferReadHandler, static_cast<void*>(this), &streamOptions) == RTAUDIO_NO_ERROR)
        {
            return audioBuffer_.open(QIODevice::ReadWrite);
        }

        OPENAUTO_LOG(error) << "[RtAudioOutput] Failed to open audio output.";
    }
    else
    {
...
void RtAudioOutput::start()
{
    ...
    if(dac_->isStreamOpen() && !dac_->isStreamRunning())
    {
        ...
        {
            OPENAUTO_LOG(error) << "[RtAudioOutput] Failed to start audio output.";
        }
    }
}
...
void RtAudioOutput::doSuspend()
{
    if(dac_->isStreamOpen() && dac_->isStreamRunning())
    {
        if(dac_->stopStream() != RTAUDIO_NO_ERROR)
        {
            OPENAUTO_LOG(error) << "[RtAudioOutput] Failed to suspend audio output.";
        }
    }
}
